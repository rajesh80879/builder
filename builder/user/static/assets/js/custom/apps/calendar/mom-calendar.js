...
var KTMOMAppCalendar = function () {
        ...
        calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick: function (arg) {
                ...
                formatArgs({
                    ...
                    subtask:arg.event.extendedProps.subtasks,
            ...
            $.ajax({
                type: "GET",
                url: '/get-all-tasks-meeting/' ,
                success: function(response){
                    var events = [];
                    console.log(response,"datataaaaa")

                    for(let i = 0; i<response.length;i++) {
                        events.push({
                            id: response[i].id,
                            title: response[i].task_title,
                            start: response[i].task_due_date,
                            end: response[i].task_due_date,
                            company: response[i].company,     
                            user: response[i].user,
                            subtasks: response[i].task_title,  
                            status: response[i].task_status,             
                        });
                    };
                    successCallback(events);
                    console.log(events,"eventttttttt") 
                },
    ...
    const handleViewEvent = () => {
        viewModal.show();
        console.log(data,"resssssssssssssss")

        // Populate view data
        viewStartDate.innerText = data.startDate;
        viewEndDate.innerText = data.endDate;
        viewEventName.innerText = data.eventName;
        viewCompany.innerText = data.company;
        viewUser.innerText = data.user;
        viewSubtask.innerText = data.subtask;
    ...
    const formatArgs = (res) => {
        console.log(res,"resssssssjjjjjjsssssss")
        data.id = res.id;
        data.eventName = res.title;
        data.user = res.user;
        data.company= res.company;
        data.subtask = res.subtask;
        data.status = res.status;
        data.startDate = res.startStr.split('T')[0];;
        data.endDate = res.startStr.split('T')[0];;
    ...
    return {
        ...
        init: function () {
            ...
            viewUser = viewElement.querySelector('[data-kt-calendar="event_user"]');
            viewCompany = viewElement.querySelector('[data-kt-calendar="event_company"]');
            viewSubtask = viewElement.querySelector('[data-kt-calendar="event_subtask"]');
